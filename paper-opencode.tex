...
designed for large language models, provides two core concepts:
...
\cite{agent-skills,skills-sh,clawhub,skill-fish,anthropic-skills} and
Agents (executors responsible for scheduling, memory, and context
management) \cite{freakwill-agents}. In practice, developers often face
...
\bottomrule\noalign{}
\endlastfoot
Skill file & lowercase + underscore/hyphen, verb phrase or agentless
noun phrase & \passthrough{\lstinline!find-skills.md!},
\passthrough{\lstinline!data-analysis.md!} \\
...
In practice, skill file names still tend to use agent-bearing noun
...
\subsubsection{File Conventions}\label{file-conventions}
...
summarize the following four security principles. For daily work and
...
Cross-script injection & Perform strict escaping and whitelist
...
\subsection{Design Techniques}\label{design-techniques}

\subsubsection{Variable Pre-setting}\label{variable-pre-setting}
...
\subsubsection{Custom Commands}\label{custom-commands}
...
OpenCode does support custom commands
(https://opencode.ai/docs/commands/), but when defined in a skill/agent
...
\subsubsection{Simple Agent Memory
Implementation}\label{simple-agent-memory-implementation}

LLMs themselves do not possess implicit recurrent memory units like
RNNs; their ``memory'' is limited to the current context window. Agent
...
startup. Below is a simple example.
...
https://www.skills.sh/sickn33/antigravity-awesome-skills/agent-memory-systems.

\subsubsection{Self-Management of
OpenCode}\label{self-management-of-opencode}
...
\subsubsection{Dialogue Example (Personality
Distillation)}\label{dialogue-example-personality-distillation}
...
\cite{brown-2020,dong-2024}. The agent can automatically reuse
...
\subsection{Tool Overview}\label{tool-overview}
...
\subsubsection{Built-in Tools}\label{built-in-tools}

OpenCode provides a series of built-in tools for large language models,
...
commonly used built-in tools (see https://opencode.ai/docs/tools/ for
details):
...
\subsection{Skill Case Studies}\label{skill-case-studies}
...
\subsubsection{Skill Factory
(skill-creator)}\label{skill-factory-skill-creator}
...
\subsubsection{Academic Paper Writing}\label{academic-paper-writing}
...
written
(https://github.com/Freakwill/opencode-agents/blob/main/template-opencode.md),
...
\subsection{Agent Example}\label{agent-example}
...
\begin{enumerate}
  \item Memory and
Evolution: Special focus on the memory mechanisms and self-evolution
capabilities of skills and agents.
  \item Security Audit Plugin: Develop a
unified security audit skill for static analysis of tools/commands,
proactively capturing potential risks.
  \item Multimodal Skills: Integrate
multimodal inputs including text, images, and audio to further expand
