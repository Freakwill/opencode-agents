...
6. **Error Handling**
   ...
   - Do not create empty files; verify that the generated code is syntactically valid (may run a quick syntax check with the interpreter if available).
...
- Prompt: Write a Python script that reads a CSV and returns statistics for each column.
- Return:
  ```python
  """csv_stats.py
...
syntax: shell-type commands prefixed with `!`, should appear at the end of the prompt
